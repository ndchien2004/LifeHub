...
class BootstrapServiceTest {
    ...
    private final AiSettings aiSettings = mock(AiSettings.class);
    private final BootstrapService service =
            new BootstrapService(settingService, reminderService, dashboardService, aiSettings);
    ...
    void returnsEverythingTheRendererNeeds() {
        ...
        when(aiSettings.isConfigured()).thenReturn(true);

        BootstrapData data = service.load();

        assertThat(data.settings()).containsEntry("app.theme", "SYSTEM");
        assertThat(data.aiConfigured())
                .as("Có API key thì bootstrap phải nói cho renderer biết (FR-AI-09)")
                .isTrue();
